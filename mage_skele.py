...
def init_sell():
    def sell_etc():
        ...
        try: 
            ...
            sell_etc_counter = 0 
            while brokenhorn or stim or aqua_ore or top_ore or amy_ore or oldneckbone or sell_etc_counter < 10:
                sell_etc_counter += 1
                print('Sell etc counter - ' + str(sell_etc_counter)) 
                try:
    ...
    time.sleep(1.5)
    sell_all_counter = 0
    while sell_all is None and sell_all_counter < 10:
        sell_all_counter += 1
        try:
    ...
    time.sleep(3)
    sell_etc()
    leave_store_counter = 0
    while leave_store is None and leave_store_counter < 10:
        leave_store_counter += 1
        try:
...
def deaggro():
    ...
    while fm is None:
        try:
            ...
            time.sleep(1)
            try:
                accept_fam = pyag.locateOnScreen("accept_fam.jpg",region=(0,50,1000,700),confidence=0.8) 
                pyag.click(accept_fam)
            except:
                pass
            press('UP',0.3)
        except Exception as e:
...
def check_unable_connect():
    ms.activate()
    try:
        if pyag.locateOnScreen('unable_connect.jpg', confidence=0.85):
            center_ok = pyag.locateCenterOnScreen('unable_ok.jpg', confidence=0.8)
            pyag.click(center_ok)
            time.sleep(2)
            ...
            press('ENTER') #Channel
            time.sleep(10)
            press('ENTER') #Character
            time.sleep(2)
            sendSequence('510510')
            press('ENTER')
            time.sleep(10)
    except:
        print('Failed to detect unable connect, still in game?')
